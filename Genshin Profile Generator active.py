...
from tkinter import Tk, Button, Frame, Scrollbar,Canvas
...
root = tk.Tk()
root.state("zoomed")
root.title("Genshin Card Choice")
...
def on_click(image_path):
    # Xóa tất cả nội dung trong canvas_two, nhưng không phá hủy nó
    if canvas.winfo_exists():
        canvas.delete("all")
    else:
        print("canvas_two đã bị phá hủy!")
    ...
    canvas_two = Canvas(root, bd=0, highlightthickness=0, width=1000, height=800)
    canvas_two.pack()
    canvas_two.create_image(0, 12, anchor="nw", image=image_photo)
    # Hiển thị ảnh đã resize (profile_edited_2.png) chồng lên
    canvas_two.create_image(0, 0, anchor="nw", image=frame_photo)
    canvas_two.create_image(241, 80, anchor="nw", image=avt_photo)
    ...
    # Chuyển đổi ảnh rỗng chứa chữ thành PhotoImage và hiển thị trên canvas_two
    text_photo = ImageTk.PhotoImage(text_image)
    canvas_two.create_image(0, 0, anchor="nw", image=text_photo)
    canvas_two.image_refs = [image_photo, frame_photo, avt_photo, text_photo]
    canvas_two.update()  # Cập nhật canvas_two nếu cần thiết
...
def get_image_files(folder, keyword="Namecard", extension=".png"):
    ...
    for root, _, files in os.walk(folder):  # Duyệt qua tất cả các thư mục con
        for file in files:
            if keyword.lower() in file.lower() and file.endswith(extension):  # Kiểm tra xem file có chứa từ khóa 'Namecard' hay không
